...
class MongoMemberRepository {
  ...
  async getAllMembers() {
    try {
      const members = await Member.find(); // Trae todos los miembros
      return members;
    } catch (error) {
      throw new Error("Error al obtener los miembros: " + error.message);
    }
  }

  async findLeanUncheckedMembers() {
    const members = await MemberModel.find({
      $or: [{ dataConfirmed: false }, { dataConfirmed: { $exists: false } }],
    })
  ...
  async find(page) {
    ...
    const members = await MemberModel.find().skip(skip).limit(pageSize);
  ...
  async findMembersBirthdayThisWeek() {
    ...
    const startOfWeek = today
      .clone()
      .day(today.day() >= 2 ? 1 : 2 - 7)
      .startOf("day"); // Today if it's Tuesday, or the previous Tuesday

    const endOfWeek = startOfWeek
      .clone()
      .add(6 - (today.day() === 1 ? 7 : 0), "days")
      .endOf("day"); // Next Monday
  ...
  async findSummary(filter) {
    ...
    if (filter.choirs === "true") {
      const services = await ServicesModel.find({ service: "COROS" }).select(
        "dni -_id"
      );
    ...
    if (filter.leadsleads === "true") {
      const services = await ServicesModel.find({ service: "DIRIGE" }).select(
        "dni -_id"
      );
      dnis.push(...services.map((x) => x.dni));
    }
    ...
    if (filter.work === "true") {
      const services = await ServicesModel.find({ service: "OBRA" }).select(
        "dni -_id"
      );
      dnis.push(...services.map((x) => x.dni));
    }
    ...
    if (filter.games === "true") {
      const services = await ServicesModel.find({ service: "JUEGOS" }).select(
        "dni -_id"
      );
      dnis.push(...services.map((x) => x.dni));
    }
    ...
    if (filter.greaterThanYear > 0) {
      const yearsAgo = moment()
        .subtract(filter.greaterThanYear, "years")
        .toDate();
      query.dateOfBirth = { $lt: yearsAgo };
    }

    if (filter.lessThanYear > 0) {
      const recentYears = moment()
        .subtract(filter.lessThanYear, "years")
        .toDate();
    ...
    if (filter.noServiceSince > 0) {
      const monthsAgo = moment()
        .subtract(filter.noServiceSince, "months")
        .toDate();
      const recentServices = await ServicesModel.find({
        date: { $gte: monthsAgo },
      }).select("dni -_id");
      const recentDnis = recentServices.map((x) => x.dni);

      // Obtener todos los DNIs
      const allDnis = (await ServicesModel.find().select("dni -_id")).map(
        (x) => x.dni
      );

      // Filtrar los DNIs que no tienen servicios recientes
      const noRecentServiceDnis = allDnis.filter(
        (dni) => !recentDnis.includes(dni)
      );
...
module.exports = new MongoMemberRepository(); // Exporta la clase completa
